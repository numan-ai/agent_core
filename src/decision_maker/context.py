...
class Context:
    ...
    def track_max_energy(self, concept_cid: str):
        self.max_heaps.append((concept_cid, []))
        
    def add_node(self, node_id):
        if node_id not in self.nodes:
            node = Node(0, node_id)
            self.nodes[node_id] = node
            for lookup_concept, max_heap in self.max_heaps:
                if self.hierarchy.is_subconcept(node_id, lookup_concept):
    ...
    def find_max_energy_node(self, concept: str):
        # TODO: use heapq.heappop instead of nlargest
        for concept, max_heap in self.max_heaps:
        return heapq.nlargest(1, self.max_heap)
...
def main():
    ...
    graph = TaskEnergyGraph("Task", hierarchy)
    for task in tasks:
