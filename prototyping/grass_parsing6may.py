...
# we can't take all concepts of any word.
# because any word has a concept of "LiteralWord", which will make the parsing so mych slower.
# or we can..
...
for word_idx in range(len(sentence)):
    aeg.add_energy(sentence[word_idx], energy=1.0, propagation=1.0)
    if word_idx < len(sentence) - 1:
        # look ahead
        aeg.add_energy(sentence[word_idx + 1], energy=1.0, propagation=1.0)
    ...
    if pattern_nodes is None:
        ...
        for pattern_node in pattern_nodes[1:]:
            aeg.add_energy(pattern_node, energy=1.0, propagation=1.0)
    else:
